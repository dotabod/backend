...
import { say } from '../say'
import { isFeatureEnabled } from './announce-features'
import { captureCosmetics } from './capture-cosmetics'
...
export const announceCapturedCosmetics = async function announceCapturedCosmetics(
  client: SocketClient
): Promise<void> {
  try {
    ...
    if (!heroRevealableStates.includes(client.gsi?.map?.game_state ?? '')) {
      return
    }
    ...
    if (!isFeatureEnabled(client, 'cosmeticsAnnounce')) {
      return
    }
    ...
    if ((await redisClient.client.get(announcedKey)) === stamp) {
      return
    }
    ...
    logger.error('[cosmetics] failed to announce captured set', { error, token: client?.token })
  }
}
