...
import type { RedisLike } from '../invalid-tokens.ts'
...
const { dbState, resetDbState } = await import('../../../db/__tests__/db-mocks.ts')
...
} = await import('../invalid-tokens.ts')
...
const makeFakeRedis = function makeFakeRedis(
...
describe('hydrateInvalidTokensFromRedis', () => {
  const makeScannerClient = function makeScannerClient(
    keys: string[],
    opts: { throws?: boolean } = {}
  ) {
    return {
