...
const { updateMmr } = await import('../update-mmr.ts')
...
describe('updateMmr (steam32Id branch)', () => {
  ...
  it('only updates client.mmr when the account being updated is the currently-active one', async () => {
    ...
    await updateMmr({
      ...
      currentMmr: 3000,
      newMmr: 3025,
      // updating the smurf, not the active account,
      steam32Id: 99_999,
    })

    // unchanged — active account is still 11111
    expect(client.mmr).toBe(5000)
    // the smurf entry is updated
    expect(client.SteamAccount[1].mmr).toBe(3025)
  })
})
