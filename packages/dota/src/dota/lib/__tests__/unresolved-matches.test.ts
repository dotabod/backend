...
import { formatTimeAgo, formatUnresolvedMatch } from '../unresolved-matches.ts'
...
const baseMatch = (overrides: Partial<UnresolvedMatch> = {}): UnresolvedMatch => ({
  ...
  kda: { assists: 7, deaths: 2, duration: 2472, kills: 4 },
  matchId: '8821007388',
  radiant_score: 32,
...
describe(formatUnresolvedMatch, () => {
  ...
  it('omits match length when duration is missing', () => {
    const match = baseMatch({ kda: { assists: 7, deaths: 2, kills: 4 } })
