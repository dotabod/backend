...
const emitInactiveOverlayState = function emitInactiveOverlayState(io: Server, token: string) {
...
const handleSocketAuth = function handleSocketAuth(socket: Socket, next: (err?: Error) => void) {
...
const handleSocketConnection = async function handleSocketConnection(socket: Socket) {
...
class GSIServer implements GSIServerInterface {
  ...
  constructor() {
    ...
    app.get('/tooltips/:channelId', async (req: Request, res: Response) => {
      ...
      if (typeof channelId !== 'string' || !/^\d+$/u.test(channelId)) {
        res.status(200).json({ status: 'ok' })
        return
      }
  ...
  async processClipDeletionQueue(): Promise<void> {
    ...
    for (const accountId of accountIds) {
      ...
      if (!slugsToDelete || slugsToDelete.size === 0) {
        // Clean up empty entry if somehow created
