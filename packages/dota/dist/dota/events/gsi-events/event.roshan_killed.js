...
export function getRoshCountMessage(props) {
    let roshCountMsg;
    switch (props.count) {
        case 1:
...
export function getNewRoshTime(res) {
...
export function generateRoshanMessage(res, lng) {
    res = getNewRoshTime(res);
    const msgs = [];
    if (res.maxS > 0) {
        msgs.push(t('roshanKilled', {
            min: res.minTime,
            max: res.maxTime,
            lng,
        }));
...
export function emitRoshEvent(res, token) {
    if (!res || !res.minDate)
        return;
    res = getNewRoshTime(res);
...
eventHandler.registerEvent(`event:${DotaEventTypes.RoshanKilled}`, {
    handler: async (dotaClient, event) => {
        ...
        const redisJson = (await redisClient.client.json.get(`${dotaClient.getToken()}:roshan`));
...
//# sourceMappingURL=event.roshan_killed.js.map