...
import { validEventTypes } from '../../../types.js';
...
// Runs every gametick
async function saveMatchData(client) {
    ...
    if (!steamServerId && !lobbyType) {
        ...
        const getDelayedDataPromise = new Promise((resolve, reject) => {
            steamSocket.emit('getUserSteamServer', client.steam32Id, (err, cards) => {
                if (err) {
                    reject(err);
                }
                else {
...
eventHandler.registerEvent(`newdata`, {
    handler: async (dotaClient, data) => {
        ...
        const hasWon = dotaClient.client.gsi?.map?.win_team && dotaClient.client.gsi.map.win_team !== 'none';
        if (hasWon)
            return;
        ...
        if (dotaClient.client.beta_tester && dotaClient.client.stream_online) {
            ...
            if (enabled)
                minimapParser.init(data, dotaClient.mapBlocker);
        }
        // Can't just !dotaClient.heroSlot because it can be 0
        const purchaser = dotaClient.client.gsi?.items?.teleport0?.purchaser;
        const playingHeroSlot = Number(await redisClient.client.get(`${dotaClient.client.token}:playingHeroSlot`));
        ...
        const { powerTreads: { enabled: treadsChatterEnabled }, } = getValueOrDefault(DBSettings.chatters, dotaClient.client.settings);
...
function handleNewEvents(data, dotaClient) {
    ...
    // Filter new events
    const newEvents = data.events?.filter(({ game_time, event_type }) => !existingEventsSet.has(`${game_time}-${event_type}`));
    if (newEvents?.length) {
