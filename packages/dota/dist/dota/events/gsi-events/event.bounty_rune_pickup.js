...
eventHandler.registerEvent(`event:${DotaEventTypes.BountyPickup}`, {
    handler: async (dotaClient, event) => {
        ...
        if (typeof matchPlayers[event.player_id]?.heroid !== 'number' ||
        ...
        const bountyHeroNames = dotaClient.bountyHeroNames.reduce((acc, cur) => {
            if (acc[cur]) {
                acc[cur]++;
            }
            else {
        ...
        dotaClient.bountyTimeout = setTimeout(() => {
            say(dotaClient.client, t('bounties.pickup', {
                emote: 'EZ Clap',
                emote2: 'SeemsGood',
...
//# sourceMappingURL=event.bounty_rune_pickup.js.map