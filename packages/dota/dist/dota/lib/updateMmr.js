...
export function tellChatNewMMR({ streamDelay, locale, token, mmr = 0, oldMmr = 0, }) {
    ...
    if (mmrEnabled && chattersEnabled && tellChatNewMMR && mmr !== 0) {
        if (newMmr !== 0) {
            ...
            setTimeout(() => {
                chatClient.say(client.name, t('updateMmr', {
...
export async function updateMmr({ tellChat = false, force = false, currentMmr, newMmr, steam32Id, channel, token, }) {
    ...
    if (!steam32Id) {
        ...
        logger.info('[UPDATE MMR] No steam32Id provided, will update the users table until they get one', {
            channel,
        });
        await supabase
            .from('users')
...
//# sourceMappingURL=updateMmr.js.map