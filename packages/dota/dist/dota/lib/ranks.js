...
export function rankTierToMmr(rankTier) {
    if (!Number(rankTier)) {
        return 0;
    }
    ...
    const rank = ranks.find((rank) => rank.image.startsWith(`${Math.floor(Number(intRankTier / 10))}${stars}`));
    // Middle of range
    return ((rank?.range[0] ?? 0) + (rank?.range[1] ?? 0)) / 2;
}
export async function lookupLeaderRank(mmr, steam32Id) {
    ...
    if (medalCache) {
        ...
        try {
            const getCardPromise = new Promise((resolve, reject) => {
                steamSocket.emit('getCard', steam32Id, (err, card) => {
                    if (err) {
                        reject(err);
                    }
                    else {
            ...
            const standing = data?.leaderboard_rank;
            ...
            const myRank = leaderRanks.find((rank) => standing <= rank.range[1]) || leaderRanks[leaderRanks.length - 1];
...
export async function getRankDetail(mmr, steam32Id) {
...
export async function getRankDescription({ locale, mmr, steam32Id, showRankMmr = true, }) {
