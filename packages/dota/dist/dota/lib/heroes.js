...
export const translatedColor = (color, lng) => {
    if (lng === 'en')
        return color;
    const props = { lng };
    switch (color) {
...
export function getHeroNameOrColor(id, index) {
...
export function getHeroByName(name, heroIdsInMatch) {
    if (!name)
        return null;
    ...
    let hero = lookInHeroes.find((h) => {
        const hasAlias = h.alias.some((alias) => alias
            .replace(/[^a-z]/gi, '')
            .toLowerCase()
            .trim() === name);
        if (hasAlias)
