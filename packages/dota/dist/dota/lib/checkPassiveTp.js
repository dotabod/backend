...
export async function checkPassiveTp(client) {
    ...
    if (Number(client.gsi?.map?.clock_time) <= 30)
        return;
    const passiveTpData = (await redisClient.client.json.get(`${client.token}:passiveTp`)) || {
    ...
    if (hasTp) {
        ...
        if (passiveTpData.told) {
            const seconds = Math.round((Date.now() - passiveTpData.told + PASSIVE_THRESHOLD_SECONDS) / 1000);
            if (deadge) {
                say(client, t('chatters.tpFromDeath', {
                    emote: 'Okayeg 👍',
                    seconds,
                    channel: `@${client.name}`,
                    lng: client.locale,
 ...
 */
async function resetPassiveTime(token) {
...
//# sourceMappingURL=checkPassiveTp.js.map