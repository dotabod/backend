...
import { emitAegisEvent } from './events/gsi-events/event.aegis_picked_up.js';
import { emitRoshEvent } from './events/gsi-events/event.roshan_killed.js';
...
export function getStreamDelay(settings) {
    return Number(getValueOrDefault(DBSettings.streamDelay, settings)) + GLOBAL_DELAY;
}
export function emitMinimapBlockerStatus(client) {
...
export async function deleteRedisData(client) {
...
export class GSIHandler {
    ...
    async emitNotablePlayers() {
        ...
        const enableCountries = getValueOrDefault(DBSettings.notablePlayersOverlayFlagsCmd, this.client.settings);
        notablePlayers({
    ...
    async updateSteam32Id() {
        ...
        if (isSameSteam32Id || isMultiAccount || isSameAccountId)
            return;
    ...
    async handleExistingAccount(res, steam32Id) {
        if (res.userId === this.client.token) {
            Object.assign(this.client, { mmr: res.mmr, steam32Id });
        }
        else {
            this.client.multiAccount = steam32Id;
            const uniqueUserIds = Array.from(new Set([...(res?.connectedUserIds ?? []), this.client.token]));
            await supabase
                .from('steam_accounts')
    ...
    async createNewSteamAccount(mmr, steam32Id) {
    ...
    async updateMMR({ scores, increase, heroName, lobbyType, matchId, isParty, heroSlot }) {
    ...
    async closeBets(winningTeam = null) {
        const matchId = await redisClient.client.get(`${this.client.token}:matchId`);
        const myTeam = (await redisClient.client.get(`${this.client.token}:playingTeam`)) ??
        ...
        const heroName = (await redisClient.client.get(`${this.client.token}:playingHero`));
        ...
        if (!this.client.gsi?.map?.dire_score &&
            ...
            if (this.client.stream_online) {
                say(this.client, t('bets.notScored', {
                    emote: 'D:',
                    lng: this.client.locale,
                    matchId,
                    key: DBSettings.tellChatBets,
        ...
        const toggleHandler = async () => {
            const treadToggleData = (await redisClient.client.json.get(`${this.client.token}:treadtoggle`));
            if (treadToggleData?.treadToggles && this.client.stream_online) {
                say(this.client, t('treadToggle', {
                    lng: this.client.locale,
    ...
    emitBlockEvent({ blockType, state }) {
...
async function maybeSendRoshAegisEvent(token) {
    const aegisRes = (await redisClient.client.json.get(`${token}:aegis`));
    const roshRes = (await redisClient.client.json.get(`${token}:roshan`));
    if (aegisRes) {
...
//# sourceMappingURL=GSIHandler.js.map