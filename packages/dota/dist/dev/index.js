...
async function postWinEventsForUsers(users, win_team = 'radiant') {
...
export async function postEventsForUsers(users, eventType) {
    const promises = users.map((user) => apiClient.post('/', {
        player: {
            activity: 'playing',
        },
        events: [
...
await fixNewUsers();
async function handleNewUser(providerAccountId, botApi) {
    if (!botApi)
        return;
    try {
        ...
        const filteredData = Object.fromEntries(Object.entries(data).filter(([key, value]) => Boolean(value)));
        let userId = null;
