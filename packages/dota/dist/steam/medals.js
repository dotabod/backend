...
export async function gameMedals(locale, currentMatchId, players) {
    ...
    try {
        ...
        const medals = cards.map((card, i) => {
            const currentMedal = medalQuery.find((temporaryMedal) => temporaryMedal.rank_tier === card.rank_tier);
            if (!currentMedal)
