...
const updateStats = (stats, match, multiplier) => {
    if (match.won) {
...
export async function getWL({ lng, channelId, mmrEnabled, startDate }) {
    if (!channelId) {
    ...
    const ranked = { win: 0, lose: 0, mmr: 0 };
    const unranked = { win: 0, lose: 0 };
