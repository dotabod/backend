...
class SetupSupabase {
    ...
    init() {
        this.channel
            ...
            if (this.IS_DEV && !this.DEV_CHANNELS.includes(payload.old.name))
                return;
            if (!this.IS_DEV && this.DEV_CHANNELS.includes(payload.old.name))
                return;
            ...
            if (this.IS_DEV && !this.DEV_CHANNELIDS.includes(payload.new?.providerAccountId))
                return;
            if (!this.IS_DEV && this.DEV_CHANNELIDS.includes(payload.new?.providerAccountId))
                return;
            ...
            if (this.IS_DEV && !this.DEV_CHANNELS.includes(payload.new.name))
                return;
            if (!this.IS_DEV && this.DEV_CHANNELS.includes(payload.new.name))
                return;
            const newObj = payload.new;
            ...
            if (newObj.key === DBSettings.commandDisable) {
                if (!client) {
                    ...
                    if (this.IS_DEV && !this.DEV_CHANNELS.includes(client.name))
                        return;
                    if (!this.IS_DEV && this.DEV_CHANNELS.includes(client.name))
                        return;
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'steam_accounts' }, async (payload) => {
            const newObj = payload.new;
            const oldObj = payload.old;
