...
twitchChat.on('msg', async function (channel, user, text, { channelId, userInfo, messageId }) {
    if (!channelId)
        return;
    // Letting one pleb in
    if (plebMode.has(channelId) &&
        !(userInfo.isMod || userInfo.isBroadcaster || userInfo.isSubscriber)) {
        plebMode.delete(channelId);
        chatClient.say(channel, '/subscribers');
        chatClient.say(channel, t('pleb', { emote: 'EZ Clap', context: 'off', name: user, lng: userInfo.locale }));
        return;
    }
    if (!text.startsWith('!'))
        return;
