import { getValueOrDefault } from '@dotabod/settings';
...
class CommandHandler {
    aliases = new Map();
    commands = new Map(); // Map for storing command information
    cooldowns = new Map(); // Map for storing command cooldowns
    bypassCooldownUsers = []; // List of users that are allowed to bypass the cooldown
    constructor() {
    ...
    async logCommand(commandName, message) {
        ...
        try {
            await db.collection('commandstats').updateOne({ command, channel, date }, {
                $set: data,
                $inc: {
                    count: 1,
                },
    ...
    async handleMessage(message) {
        ...
        if (options.onlyOnline && !message.channel.client.stream_online) {
            chatClient.say(message.channel.name, t('notLive', { emote: 'PauseChamp', lng: message.channel.client.locale }));
            return;
        }
        ...
        if (this.isOnCooldown(commandName, options.cooldown ?? defaultCooldown, message.user, message.channel.id)) {
    ...
    isOnCooldown(command, cooldown, user, channelId) {
    ...
    updateCooldown(command, cooldown, channelId) {
        // Check if the command has a cooldown
        if (cooldown === 0) {
