...
commandHandler.registerCommand('hero', {
    ...
    handler: async (message, args, command) => {
        ...
        if (!gsi || !client.gsi?.map?.matchid)
            return handleNotPlaying(message);
        try {
            const { player, hero, playerIdx } = await findAccountFromCmd(client.gsi, args, client.locale, command);
            await getHeroMsg({
                heroId: hero?.id ?? 0,
                channel,
                hasHero: !!hero?.id,
...
function handleNotPlaying(message) {
    chatClient.say(message.channel.name, t('notPlaying', { emote: 'PauseChamp', lng: message.channel.client.locale }));
}
function speakHeroStats({ heroNameOrColor, hasHero, win, lose, channel, lng, }) {
    const total = (win || 0) + (lose || 0);
    const timeperiod = t('herostats.timeperiod.days', { count: 30, lng });
    if (!total) {
        chatClient.say(channel, t(hasHero ? 'herostats.noneStreamer' : 'herostats.noneColor', {
            lng,
    ...
    chatClient.say(channel, t(hasHero ? 'herostats.winrateStreamer' : 'herostats.winrateColor', {
        lng,
        heroName: heroNameOrColor,
...
async function getHeroMsg({ channel, steam32Id, heroNameOrColor, token, hasHero, lng, heroId, }) {
    ...
    sockets[0]
        .timeout(15000)
        .emit('requestHeroData', { allTime: false, heroId, steam32Id }, (err, response) => {
        if (!response)
            return;
