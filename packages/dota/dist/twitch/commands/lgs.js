...
commandHandler.registerCommand('lgs', {
    ...
    handler: async (message, args) => {
        ...
        `)
            ...
            .single();
        if (!lg) {
            chatClient.say(message.channel.name, t('noLastMatch', { emote: 'PauseChamp', lng: message.channel.client.locale }));
            return;
        }
        const returnMsg = [];
        returnMsg.push(lg.won
        ...
        if (kda) {
            ...
            returnMsg.push(t('lastgamescore.kda', {
                lng: message.channel.client.locale,
                heroName: getHero(lg.hero_name)?.localized_name ??
                    t('unknown', { lng: message.channel.client.locale }),
                kdavalue: kdaMsg,
            }));
        }
        // calculate the time difference in minutes between created_at and updated_at
        const lasted = Math.floor((new Date(lg.updated_at).getTime() - new Date(lg.created_at).getTime()) / 1000 / 60);
        returnMsg.push(t('lastgamescore.duration', { minutes: lasted, lng: message.channel.client.locale }));
        if (lg.is_party)
