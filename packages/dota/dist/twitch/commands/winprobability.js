...
commandHandler.registerCommand('winprobability', {
    ...
    handler: async (message) => {
        ...
        if (!apiCooldown[channel] || Date.now() - apiCooldown[channel] >= API_COOLDOWN_SEC * 1000) {
            try {
                ...
                if (lastWinRate &&
                    ...
                    const winRate = Math.floor((isRadiant ? lastWinRate.winRate : 1 - lastWinRate.winRate) * 100);
                    WinRateCache[channel] = {
                        winRate,
                        emote: winRate > 50 ? 'Pog' : 'BibleThump',
                        gameTime: lastWinRate.time,
                        remainingCooldown: Math.floor((API_COOLDOWN_SEC * 1000 - (Date.now() - apiCooldown[channel])) / 1000),
                    };
                }
                else {
        ...
        if (WinRateCache[channel]) {
            WinRateCache[channel].remainingCooldown = Math.floor((API_COOLDOWN_SEC * 1000 - (Date.now() - apiCooldown[channel])) / 1000);
        }
...
//# sourceMappingURL=winprobability.js.map