...
commandHandler.registerCommand('np', {
    ...
    handler: async (message, args) => {
        ...
        const { user: { name: chatterName }, channel: { client, name: channel, id: twitchChannelId }, } = message;
        if (!client.steam32Id) {
        ...
        async function addRemoveHandler() {
            if (addOrRemove === 'add') {
                ...
                try {
                    await db.collection('notablePlayers').updateOne({ account_id: Number(forSteam32Id), channel: twitchChannelId }, {
                        $set: {
            ...
            if (addOrRemove === 'remove') {
                ...
                try {
                    ...
                    if (removed.deletedCount) {
                        chatClient.say(channel, t('npRemoved', { steamid: forSteam32Id, lng: message.channel.client.locale }));
                    }
                    else {
                        chatClient.say(channel, t('npUnknown', { steamid: forSteam32Id, lng: message.channel.client.locale }));
                    }
                    return;
                }
                finally {
        ...
        if (commandHandler.hasPermission(message.user, 2) &&
        ...
        const enableCountries = getValueOrDefault(DBSettings.notablePlayersOverlayFlagsCmd, client.settings);
        notablePlayers({
            ...
            .catch((e) => {
            chatClient.say(channel, e?.message ?? t('gameNotFound', { lng: message.channel.client.locale }));
        });
    },
});
