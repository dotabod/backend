...
async function getItems({ token, packet, args, locale, command, }) {
    ...
    if (isSpectator(packet)) {
        itemList =
            items &&
                Object.values(items)
                    .map((itemN) => {
                    const item = itemN;
                    const itemShortname = item.name.replace('item_', '');
                    const itemFound = DOTA_ITEMS[itemShortname];
                    const itemName = itemFound && 'dname' in itemFound && itemFound.dname;
        ...
        const steamServerId = packet?.map?.matchid &&
    ...
    if (!itemList || !itemList.length) {
        throw new CustomError(t('heroItems.empty', {
