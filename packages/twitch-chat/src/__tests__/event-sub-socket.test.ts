...
import { EventsubSocket, FakeWebSocket, isEventsubConnected } from './shared-mocks.ts'
...
describe('EventsubSocket lifecycle', () => {
  ...
  it('dispose() makes the socket inert: a late close neither flips the flag nor reconnects', async () => {
    ...
    // unchanged by the disposed socket
    expect(isEventsubConnected()).toBeTruthy()
    // no reconnect
    expect(FakeWebSocket.instances).toHaveLength(1)
  })
  ...
  it('caps the reconnect backoff so the delay stays bounded', async () => {
    ...
    // 1 initial + 25 reconnects
    expect(FakeWebSocket.instances).toHaveLength(26)
    sock.dispose()
  })
