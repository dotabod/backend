...
import { isBroadcasterBeingDisabled } from './disable-cache'
import { emitChatMessage, hasDotabodSocket } from './utils/socket-manager'
...
const fitTwitchChatMessage = function fitTwitchChatMessage(message: string): string {
  ...
  const trailingLink = / · (?:https?:\/\/)?\S+\.\S+(?: · .*)?$/u.exec(message)?.[0] ?? ''
...
const makeDistinctTwitchChatMessage = function makeDistinctTwitchChatMessage(
  message: string
): string {
...
export const clearDedupeCache = function clearDedupeCache(): void {
...
const extractUserInfo = function extractUserInfo(
  badges: {
    [key: string]: string
  }[],
...
export const handleChatMessage = async function handleChatMessage(
  ...
  if (messageText.startsWith('@') && messageText.includes('!')) {
    ...
    messageText = messageText.replace(/^[^ ]+/u, '').trim()
  }
