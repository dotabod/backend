...
describe(computeReconnectDelay, () => {
  ...
  const upper = { baseMs: 5000, maxMs: 300_000, random: () => 1 }
  // random: () => 0 yields the lower bound (exp / 2).
  const lower = { baseMs: 5000, maxMs: 300_000, random: () => 0 }

  it('grows exponentially up to the cap', () => {
    expect(computeReconnectDelay(1, upper)).toBe(5000)
  ...
  it('never exceeds the cap, even for absurd attempt counts', () => {
    ...
    expect(computeReconnectDelay(1000, upper)).toBe(300_000)
  })

  it('applies equal jitter — delay never collapses below half the window', () => {
    expect(computeReconnectDelay(1, lower)).toBe(2500)
    expect(computeReconnectDelay(2, lower)).toBe(5000)
    expect(computeReconnectDelay(7, lower)).toBe(150_000)
  })

  it('treats attempts < 1 as the first attempt', () => {
    expect(computeReconnectDelay(0, upper)).toBe(5000)
    expect(computeReconnectDelay(-3, upper)).toBe(5000)
  })
