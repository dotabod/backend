...
BEGIN
  ...
  IF previous_interval IS DISTINCT FROM '100'::jsonb
    ...
    RAISE EXCEPTION 'Unexpected polling interval; inspect the tenant before applying this trial';
  END IF;

  UPDATE _realtime.extensions
  SET settings = jsonb_set(settings, '{poll_interval_ms}', '250'::jsonb, false),
      updated_at = now()
  WHERE tenant_external_id = 'realtime-dev' AND type = 'postgres_cdc_rls'
    AND settings->'poll_interval_ms' = '100'::jsonb;
END
$trial$;
