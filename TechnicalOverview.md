...
- **Authentication**: Document Kick OAuth/token flow with chat read/write scopes (or equivalent), target endpoints for live status (`/api/v2/channels/{channel}` or current live-status API), and chat send/receive. Refresh at 50–70% of token lifetime and handle revocation; store as `provider: 'kick'` with channel metadata in Supabase `accounts`.
...
- **Chat + commands**: Map existing chat command handlers to Kick transport; normalize roles (channel owner → broadcaster, moderator → moderator, subscriber/supporter → subscriber) and tune rate limits to Kick chat caps (start conservatively at ~10 msgs/30s and adjust once documented limits are confirmed).
